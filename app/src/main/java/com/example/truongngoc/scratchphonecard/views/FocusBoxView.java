...
public class FocusBoxView extends View {
    ...
    private static final int BIG_BUFFER = 60;
    private static final int LEFT_DIVIDER = 2;
    ...
    /**
     * Calculate and create the box the desired bitmap should be inside
     *
     * @return {@link Rect} that represents the box
     */
    private Rect getBoxRect() {

        // initialize the focus rectangle if the rectangle is current null
        if (box == null) {
            ...
            int left = (ScrRes.x - width) / LEFT_DIVIDER;
            int top = (ScrRes.y - height) / TOP_DIVIDER;
    ...
    /**
     * Update the box(size) when use interact with the box
     *
     * @param dW
     * @param dH
     */
    private void updateBoxRect(int dW, int dH) {
        ...
        int leftOffset = (ScrRes.x - newWidth) / LEFT_DIVIDER;

        int topOffset = (ScrRes.y - newHeight) / TOP_DIVIDER;
